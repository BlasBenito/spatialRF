...
The relationship between the selected interaction and the response variable, as shown below, indicates that there is an important threshold above which plant richness increases dramatically, so it seems that `rf_interactions()` made a good suggestion.

```{r, echo = FALSE, fig.width=5, fig.height=3.5}
ggplot2::ggplot(
    data = plant_richness_df,
    ggplot2::aes_string(
      x = "climate_bio1_average_X_bias_area_km2",
      y = "richness_species_vascular",
      color = "richness_species_vascular"
    )
    ) +
...
I have explained above that models fitted with `rf()`, `rf_tuning()`, or `rf_spatial()` have a performance slot with several performance measures.
...
```{r, echo = FALSE}
kableExtra::kable(comparison$df.aggregated) %>%
  kable_paper("hover", full_width = F)
```
