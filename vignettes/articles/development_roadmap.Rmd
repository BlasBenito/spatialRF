...
This document outlines the planned modernization of `spatialRF`, organized into logical phases to minimize disruption and maximize coherence.
...
### 1.1 Dependency Modernization (version 1.2.0)

- DONE Replace `foreach` + `doParallel` with `future` + `future.apply`
  - More flexible execution strategies
  - Better control over parallel backends

- DONE Add `progressr` for progress reporting
  - Works seamlessly with `future`
  - User-configurable progress handlers

- DONE Remove `dplyr` and `tidyr` dependencies
  - Use base R equivalents to reduce dependency footprint
  - Improve installation reliability

- DONE Replace `magrittr` pipe (`%>%`) with base pipe (`|>`)
  - Native R syntax

- DONE Remove `huxtable` from print functions to reduce dependency footprint.

- DONE Replace `@importFrom package function` with `package::function()` calls throughout the package. Right now the only @importFrom are `foreach %dopar%` (to be removed when `future` comes), `ggplot2 .data`, `patchwork plot_annotation wrap_plots` (required to make patchwork's `|` and `/` work), `ranger ranger` (required for `stats::predict()`), and `foreach %do%`

- DONE Add package `roxyglobals` and its `@autoglobal` tag across all functions, and remove declarations of global variables (example: `x <- NULL`).

### 1.2 Function Delegation

- Replace `auto_cor()` and `auto_vif()` with `collinear` package calls
...
- Use ellipsis for `ranger` arguments and deprecate argument `ranger.arguments``
- `num.trees` → `n_trees`
...
- Create wrapper functions maintaining old API with `.Deprecated()`
