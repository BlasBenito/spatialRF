...
#' tmp = robustscale(tmp)
#' boxplot(tmp$data)
scale_robust <- function(data, dim=2, center=TRUE, scale=TRUE,
                        preserveScale = TRUE){
  medians = NULL
  if(center){
...
auto_cor <- function(
  ...
  while(i <= ncol(x.cor)){
    ...
    #select variables with cor below
  ...
  #return output
  output.list <- list()
  output.list$df <- as.data.frame(x.cor)
  output.list$vars <- colnames(x.cor)
  output.list
...
rank_spatial_predictors <- function(
  ...
  if(multicollinearity.filter != "none"){
    spatial.predictors.order.selected <- spatial.predictors.order.selected[spatial.predictors.order.selected$spatial.predictors.name %in% multicollinearity.df$vars, ]
  }
