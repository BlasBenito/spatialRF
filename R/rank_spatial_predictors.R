...
#' @description Ranks spatial predictors generated by [mem_multithreshold()] by their effect in reducing the Moran's I of the model residuals (`ranking.method = "effect"`), or by their own Moran's I (`ranking.method = "moran"`).
#'
...
rank_spatial_predictors <- function(
  ...
  spatial.predictors.order <- foreach::foreach(
    ...
    if(ranking.method == "moran"){
      ...
      out.i <- data.frame(
        spatial.predictors.name = spatial.predictors.name.i,
        ranking.criteria = m.i$test$moran_i,
  ...
  out.list$spatial_predictors_df <- spatial.predictors.df[, spatial.predictors.order$spatial.predictors.name]
