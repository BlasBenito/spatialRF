...
The function `rf_tuning()` returns a model fitted with the same data as the original model, but using the best hyperparameters found during tuning. Model tuning has helped to a very small improvement in performance measures (+ `r round(model.non.spatial.tuned$tuning$metric.gain, 3)` R squared), so from here, we can keep working with `model.non.spatial.tuned`.
