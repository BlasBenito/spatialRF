...
#'  df <- get_response_curves(rf.model)
#'  head(df)
#'
#' }
...
get_response_curves <- function(
  ...
  if(sum(variables %in% colnames(data)) != length(variables)){
    ...
    stop("Variable names in 'variables' must be column names of model$ranger_arguments$data.")
  ...
  for(variable.i in variables){
    ...
    other.variables <- setdiff(model$ranger_arguments$predictors.names, variable.i)
