...
rf_tuning <- function(
  ...
  tuning <- foreach::foreach(
  ...
  ) %dopar%
    {
      ...
      m.i.performance <- m.i.performance[
        m.i.performance$model == "Testing",
        c("metric", "median")
      ]
      ...
      if (inherits(model, "rf_spatial")) {
        ...
        moran.i.interpretation <- m.i$residuals$autocorrelation$per.distance$interpretation[
          1
        ]

        #getting performance
        m.i.performance <- data.frame(
          r.squared = m.i.performance[
            m.i.performance$metric == metric,
            "median"
          ],
  ...
  metric.name <- colnames(tuning)[
    !(colnames(tuning) %in%
      c("num.trees", "mtry", "min.node.size", "moran.i.interpretation"))
  ]
  ...
  if (inherits(model, "rf_spatial")) {
    ...
    if (nrow(tuning) == 0) {
      message(
        "This spatial model cannot be tuned, all possible results increase spatial autocorrelation of the residuals"
      )
  ...
  if (verbose) {
    message(
      paste0(
        ...
        " = ",
        new.performance,
        ") performs equal or worse than the original one (",
        metric.name,
        " = ",
        old.performance,
