...
ggplot2::ggplot() +
  ...
  ggplot2::theme_bw() +
  ggplot2::labs(fill = "Plant richness") +
...
This plot is telling us that the spatial autocorrelation of the residuals is highly positive for the distances 0 and 1500, and from there it decays until becoming neutral at 3000km. Here we can say that the model is missing something.

### Fitting a spatial model

The non-spatial model fitted above can be converted into a spatial model easily with **rf_spatial()**.
...
The plot below compares the Moran's I of the residuals of the spatial (green) and non spatial (purple) models. It shows that **rf_spatial()** has managed to reduce the spatial autocorrelation (measured via [Moran's I](https://en.wikipedia.org/wiki/Moran%27s_I)) of the model residuals for the three selected distances.
...
p1 | p2
```
Let's check the five most important variables of the spatial model.
...
Three of these are spatial predictors generated by **rf_spatial()**. How do these spatial predictors look?
