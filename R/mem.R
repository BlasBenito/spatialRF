...
#' @details Takes the distance matrix `x`, double-centers it with [double_center_plants_distance()], applies \link[base]{eigen}, and returns eigenvectors with positive normalized eigenvalues (a.k.a Moran's Eigenvector Maps, or MEMs). These MEMs are later used as spatial predictors by [rf_spatial()].
...
mem <- function(
  ...
  distance.matrix.double.centered <- double_center_plants_distance(
