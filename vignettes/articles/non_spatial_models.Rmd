...
distance_thresholds <- c(100, 1000, 2000, 4000, 8000)
...
You can now create your own distance matrices from spatial coordinates using the [`distance_matrix()`](https://blasbenito.github.io/spatialRF/reference/distance_matrix.html) function.
:::
...
# Convert to km
my_distance_matrix <- floor(my_distance_matrix / 1000)
...
interactions$screening |>
  dplyr::mutate(
    ...
    max.cor.with.predictors = round(max.cor.with.predictors, 3)
  ) |>
  DT::datatable()
```
