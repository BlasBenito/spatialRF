...
The package includes an example dataset named **plant_richness_df**, a data frame with plant species richness and predictors for 227 ecoregions in the Americas, and a distance matrix among the ecoregion edges named, well, **distance_matrix**.
...
The spatial autocorrelation of the residuals of `model.non.spatial`, measured with [Moran's I](https://en.wikipedia.org/wiki/Moran%27s_I), can be plotted with `plot_moran()`

```{r, fig.width=6, fig.height=3, message=FALSE, warning=FALSE}
plot_moran(model.non.spatial, verbose = FALSE)
```
...
p1 <- ggplot2::ggplot() +
  ...
  ggplot2::geom_point(
    ...
    ggplot2::aes(
      x = x,
      y = y,
      color = spatial_predictor_1500_1
    ),
    size = 2.5
  ) +
  ggplot2::scale_color_viridis_c(direction = -1) +
  ...
  ggplot2::scale_x_continuous(limits = c(-170, -30)) +
  ggplot2::scale_y_continuous(limits = c(-58, 75))  +
  ggplot2::ggtitle("Variable: spatial_predictor_1500_1") + 
...
Model tuning can be done on out-of-bag (`method = "oob"`) or spatial cross-validation (`method = "spatial.cv"`) R squared values.
...
Notice that the object returned by `rf_tuning()` is a model fitted with the same data as the original model, but using the best hyperparameters found during tuning. Now we can compare the original model with the tuned one. If `rf_tuning()` fails to find a best set of hyperparameters, the original model is returned, and a warning is issued.
