...
select_spatial_predictors_sequential <- function(
  ...
  p <- progressr::progressor(
    along = seq_len(length(spatial.predictors.ranking))
  )

  # Parallel execution with progress
  optimization.df_list <- future.apply::future_lapply(
    X = seq_len(length(spatial.predictors.ranking)),
  ...
  # Combine results
  optimization.df_combined <- do.call(rbind, optimization.df_list)

  #preparing optimization df
  optimization.df <- data.frame(
    spatial.predictor.name = spatial.predictors.ranking,
    spatial.predictor.index = optimization.df_combined$spatial.predictor.index,
    moran.i = optimization.df_combined$moran.i,
    p.value = optimization.df_combined$p.value,
    p.value.binary = ifelse(optimization.df_combined$p.value >= 0.05, 1, 0),
    r.squared = optimization.df_combined$r.squared,
    penalization.per.variable = (1 / nrow(optimization.df_combined)) *
      optimization.df_combined$spatial.predictor.index
  )
