...
The function [rf_evaluate()](https://blasbenito.github.io/spatialRF/reference/rf_evaluate.html) overcomes the limitations of these performance scores by providing honest performance scores on data not used to train the model by applying spatial cross-validation. It separates the training data into a number of spatially independent training and testing folds, fits a model on each training fold, predicts over each testing fold, and computes statistics of performance measures across folds. Let's see how it works.
