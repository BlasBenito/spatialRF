#' @title optimization_function
#' @description optimizes the selection of spatial predictors using two different methods: "moran.i", and "p.value". The user shouldn't use this function. See Details.
#' @param x Optimization data frame generated internally by [select_spatial_predictors_optimized] and [select_spatial_predictors_optimized]. Default: NULL
...
#' @details The method "moran.i" tries to maximize `1 - Moran's` I while taking into account the R-squared of the model and a penalization on the number of introduced spatial predictors through the expression `(1 - Moran's I) + w1 * r.squared - w2 * penalization`. The method "p.value" uses a binary version of the p-values of Moran's I (1 if >= 0.05, 0 otherwise), and uses the expression `max(1 - Moran's I, binary p-value) + w1 * r.squared - w2 * penalization`. The "moran.i" method allows to include more spatial predictors than "p.value"
