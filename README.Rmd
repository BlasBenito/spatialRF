...
```
Here `rf_interactions()` one candidate interaction. Notice that interactions computed via multiplication are named `a..x..b`, while interactions computed via PCA are named `a..pca..b`. The function cannot say whether an interaction *makes sense*, and it is up to the user to choose wisely whether to select an interaction or not.

The function returns the data, including the selected interactions, required to fit models with the functions within the package.
...
Now, let's see if this model needs spatial predictors (beyond this point, SDMs are no longer possible, because the spatial predictors cannot be represented as rasters, as they are mathematical representations of the distances among the training points).
