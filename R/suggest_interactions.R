...
#' @description Suggests candidate variable interactions by selecting the variables above a given importance threshold (argument `importance.threshold`) from a model and combining them in pairs through multiplication (`a * b`). For each variable interaction, a model including all the predictors plus the interaction is fitted, and it's R squared is compared with the R squared of a model fitted only with the original variables. This model with the original variables can either be provided through the argument `model`, or fitted with [rf_repeat()] by providing the arguments `data`, `dependent.variable.name`, and `predictor.variable.names`. We advise the users not to use variable interactions blindly. Most likely, only one or a few of the suggested interactions may make sense from a domain expertise standpoint.
