...
Random Forests already takes into account variable interactions of the form "variable `b` becomes important when `a` is higher than x" in every regression tree of the forest. However, with Random Forest we can also use the more traditional concept of variable interaction across the whole ranges of two predictors, as in `a * b`, the common way to define them in regression models. 

The function `rf_interactions()` helps the user to identify potential variable interactions. It does so by testing all possible interactions among predictors, using each one of them in a separate model (y ~ predictors + interaction), and suggesting the ones with the higher potential contribution to the model's R squared.
...
```
Here the function suggests four candidate interactions, shows the variable importance of each interaction within the model, and their contribution to the model's R squared (column "R2 improvement"). Of course, the function cannot say whether an interaction *makes sense*, and it is up to the user to choose wisely (domain expertise is key here) whether to select an interaction or not.
...
```{r}
plant_richness_df[, "climate_bio1_average::bias_area_km2"] <- interactions$columns[, "climate_bio1_average::bias_area_km2"]
predictor.variable.names <- c(predictor.variable.names, "climate_bio1_average::bias_area_km2")
```
