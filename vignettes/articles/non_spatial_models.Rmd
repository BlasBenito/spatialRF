...
# The columns 'x' and 'y' are automatically detected as longitude and latitude
...
- Auto-detects coordinate columns from data frames with flexible naming (x/X/lon/long/longitude/longitud for longitude, y/Y/lat/latitude/latitud for latitude)
- Automatically uses geodesic distances for geographic coordinates and Euclidean distances for projected coordinates
- Supports both data frames and sf objects
- Uses `sf::st_distance()` internally for accurate distance calculations
- Returns distances in kilometers (always) as a matrix compatible with all spatialRF functions

```{r, eval=FALSE}
# Other input formats:

# From a data frame with different column names
coords_df <- data.frame(
  longitude = c(-122.4, -73.9, -87.6),
  latitude = c(37.8, 40.7, 41.9),
  city = c("SF", "NYC", "Chicago")
)
dm <- distance_matrix(data = coords_df)

# From sf objects (requires sf package)
library(sf)
pts <- st_as_sf(coords_df, coords = c("longitude", "latitude"), crs = 4326)
dm <- distance_matrix(data = pts)
```
