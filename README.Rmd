...
Since this function fits many alternative random forest models, it can be run in parallel if a cluster is provided. The function does not shutdown the cluster, but you can stop it with `parallel::stopCluster(cl = cluster)`. However, we'll leave it up for other functions in this tutorial.
