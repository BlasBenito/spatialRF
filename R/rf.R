...
#' @param predictors.names (required if `model` is `NULL`; character vector with column names of `data`) Character vector with the names of the predictive variables. Every element of this vector must be in the column names of `data`. Optionally, the result of [mc_auto_cor()] or [mc_auto_vif()]. Default: `NULL`
...
#' @param distance.thresholds  (optional; numeric vector with distances in the same units as `distance.matrix`) Numeric vector with neighborhood distances. All distances in the distance matrix below each value in `distance.thresholds` are set to 0 for the computation of Moran's I. If `NULL`, it defaults to seq(0, max(distance.matrix), length.out = 4). Default: `NULL`
...
#' @param verbose (optional; logical) If TRUE, messages generated during the execution of the function are displayed. Default: `TRUE`
...
rf <- function(
  ...
  #checking if response.name and predictors.names are in colnames(data)
  ...
  #subset data
  data <- data[, c(response.name, predictors.names)]
  ...
  #computing case weights if response.name is binary
